...
###############################################################################################			
			...
			system ("cp $ctl_dir/* .");
			my $OriTreeFile = `ls *.tree`;
			my $codemlFile = `ls *.ctl`;
			chomp $OriTreeFile;
			chomp $codemlFile;
			system ("tree_doctor -P $samples_present -n -N -t $OriTreeFile > tmpPruned.tree");
			...
			system ("sed -i.bak 's/paml.tree/tmpPruned.tree/g' $codemlFile");
			
			### 1. NULL MODEL
			system ("sed -i.bak 's/paml.out/$transcript.out/g' $codemlFile");
			system ("codeml");
			system ("sed -i.bak 's/$transcript.out/paml.out/g' $codemlFile");
