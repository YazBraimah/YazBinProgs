...
###############################################################################################			
			...
			### 1. NULL MODEL
			system ("sed -i.bak 's/paml.out/$transcript.out/g' codeml.ctl");
			system ("codeml");
			system ("sed -i.bak 's/$transcript.out/paml.out/g' codeml.ctl");
			...
			system ("sed -i.bak 's/model = 0/model = 2/g' codeml.ctl");	
				...
				# edit $codemlFile 
				system ("sed -i.bak 's/paml.out/$transcript.$strain.br.out/g' codeml.ctl");
				# run
				system ("codeml");
				# back to normal
				system ("sed -i.bak 's/$transcript.$strain.br.out/paml.out/g' codeml.ctl");
			...
			system ("sed -i.bak 's/fix_omega = 0/fix_omega = 1/g' codeml.ctl");
				...
				system ("sed -i.bak 's/$strain/$strain\#1/g' tmpPruned.tree");
				# edit $codemlFile 
				system ("sed -i.bak 's/paml.out/$transcript.$strain.brSt.H0.out/g' codeml.ctl");
				### run
				system ("codeml");
				### swith to H1
				system ("sed -i.bak 's/$transcript.$strain.brSt.H0.out/$transcript.$strain.brSt.H1.out/g' codeml.ctl");
				system ("sed -i.bak 's/fix_omega = 1/fix_omega = 0/g' codeml.ctl");
				### run 
				system ("codeml");
				### back to normal
				system ("sed -i.bak 's/$transcript.$strain.brSt.H1.out/paml.out/g' codeml.ctl");
				system ("sed -i.bak 's/$strain\#1/$strain/g' tmpPruned.tree");
