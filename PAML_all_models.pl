...
###############################################################################################			
			...
			system ("sed -i.bak 's/model = 0/model = 2/g' codeml.ctl");	
				...
				# edit codeml.ctl 
				system ("sed -i.bak 's/paml.out/$transcript.br_Damr.out/g' codeml.ctl");
				# run & fetch
				system ("codeml");
				my $Damr_br_lnL = `grep -w -m 1 'lnL' $transcript.br_Damr.out | awk '{print \$5}'`;
				# back to normal
				system ("sed -i.bak 's/Damr \#1/Damr /g' ANLV.tree");
				system ("sed -i.bak 's/$transcript.br_Damr.out/paml.out/g' codeml.ctl");
			system ("mv $transcript.br_Damr.out PAML.output");
			
				# 2b. Dnov
				# edit tree
				system ("sed -i.bak 's/Dnov /Dnov \#1/g' ANLV.tree");
				# edit codeml.ctl 
				system ("sed -i.bak 's/paml.out/$transcript.br_Dnov.out/g' codeml.ctl");
				# run & fetch
				system ("codeml");
				my $Dnov_br_lnL = `grep -w -m 1 'lnL' $transcript.br_Dnov.out | awk '{print \$5}'`;
				# back to normal
				system ("sed -i.bak 's/Dnov \#1/Dnov /g' ANLV.tree");
				system ("sed -i.bak 's/$transcript.br_Dnov.out/paml.out/g' codeml.ctl");
			system ("mv $transcript.br_Dnov.out PAML.output");
				
				# 2c. Dvir
				# edit tree
				system ("sed -i.bak 's/Dvir /Dvir\#1 /g' ANLV.tree");
				# edit codeml.ctl 
				system ("sed -i.bak 's/paml.out/$transcript.br_Dvir.out/g' codeml.ctl");
				# run & fetch
				system ("codeml");
				my $Dvir_br_lnL = `grep -w -m 1 'lnL' $transcript.br_Dvir.out | awk '{print \$5}'`;
				# back to normal
				system ("sed -i.bak 's/Dvir \#1/Dvir /g' ANLV.tree");
				system ("sed -i.bak 's/$transcript.br_Dvir.out/paml.out/g' codeml.ctl");
			system ("mv $transcript.br_Dvir.out PAML.output");
			...
			print $out 			"$transcript\t$H0_omega\t$H0_lnL\t$Damr_br_lnL\t$Dnov_br_lnL\t$Dvir_br_lnL\t$M1_lnL\t$M2_lnL\t$M7_lnL\t$M8_lnL\t$Damr_brSt_H0\t$Damr_brSt_H1\t$Dnov_brSt_H0\t$Dnov_brSt_H1\t$Dvir_brSt_H0\t$Dvir_brSt_H1\n";
		...
		if ($run_PAML_flag){
				system ("echo 'TRANSCRIPT\tpaml_dNdS\tH0_lnL\tDamr_br_lnL\tDnov_br_lnL\tDvir_br_lnL\tDamr_brSt_H0\tDamr_brSt_H1\tDnov_brSt_H0\tDnov_brSt_H1\tDvir_brSt_H0\tDvir_brSt_H1' | cat - PAML.output/*.comp_paml.txt > PAML.output/comp_paml.all.txt");
				system ("sed -i.bak '/^\$/d' PAML.output/comp_paml.all.txt");
				system ("rm *bak PAML.output/*bak");
