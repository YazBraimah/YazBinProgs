...
#
#	--run_PAML					Perform PAML codeml (will run all models)
#
#		--ctl_dir				directory with control and tree files
#		
#		*** Running PAML will execute the default NULL, i.e. model = 0, NSsites = 0. To execute the branch model and/or the branch-site model, specify as follows:
#
#		--branch	(optional)
#		--branchSite	(optional)	
#		--restrict_samples			(required if --branch or --branchSite is specificied)
#								file with species/line IDs to use in branch-type PAML tests (comma-separated; must match last ID in FASTA file)
#
#
#	--help						
#
...
###############################################################################################			
			...
			system ("sed -i.bak 's/paml.phy/$transcript.phy/g' $codemlFile");
			
			### 1. NULL MODEL
			system ("sed -i.bak 's/paml.out/$transcript.H0.out/g' $codemlFile");
			system ("codeml");
			system ("sed -i.bak 's/$transcript.H0.out/paml.out/g' $codemlFile");
			...
			system ("sed -i.bak 's/model = 0/model = 2/g' $codemlFile");	
				...
				# edit tree
				system ("sed -i.bak 's/$strain/$strain\#1/g' $treeFile");
				# edit $codemlFile 
				system ("sed -i.bak 's/paml.out/$transcript.$strain.br.out/g' $codemlFile");
				# run
				system ("codeml");
				# back to normal
				system ("sed -i.bak 's/$transcript.$strain.br.out/paml.out/g' $codemlFile");
				system ("sed -i.bak 's/$strain\#/$strain/g' $treeFile");
			system ("mv $transcript.$strain.br.out PAML.output");
			system ("sed -i.bak 's/model = 2/model = 0/g' $codemlFile");
			}
			
			if ($branchSite_flag) {
			### 3. BRANCH-SITE MODELS
			system ("sed -i.bak 's/NSsites = 0/NSsites = 2/g' $codemlFile");
			system ("sed -i.bak 's/model = 0/model = 2/g' $codemlFile");
			system ("sed -i.bak 's/fix_omega = 0/fix_omega = 1/g' $codemlFile");
				
				# edit tree
				system ("sed -i.bak 's/$strain/$strain\#1/g' $treeFile");
				# edit $codemlFile 
				system ("sed -i.bak 's/paml.out/$transcript.$strain.brSt.H0.out/g' $codemlFile");
				### run
				system ("codeml");
				### swith to H1
				system ("sed -i.bak 's/$transcript.$strain.brSt.H0.out/$transcript.$strain.brSt.H1.out/g' $codemlFile");
				system ("sed -i.bak 's/fix_omega = 1/fix_omega = 0/g' $codemlFile");
				### run 
				system ("codeml");
				### back to normal
				system ("sed -i.bak 's/$transcript.$strain.brSt.H1.out/paml.out/g' $codemlFile");
				system ("sed -i.bak 's/$strain\#1/$strain/g' $treeFile");

			system ("sed -i.bak 's/NSsites = 2/NSsites = 0/g' $codemlFile");
			system ("sed -i.bak 's/model = 2/model = 0/g' $codemlFile");
			...
			## Clean-up
			system ("sed -i.bak 's/$transcript.phy/paml.phy/g' $codemlFile");
 			system ("rm $transcript.*P.afa");
 			system ("rm $transcript.phy");
			system ("rm $treeFile");
			system ("rm $codemlFile");
			}
